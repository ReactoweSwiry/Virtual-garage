...
export default function MaintenanceEventForm() {
  ...
  const { data: fetchedData, isLoading: isFetchingInitialData } = useQuery(
    {
      ...
      queryFn: () =>
        mode === 'edit' && actionId
          ? getMaintenanceDetails(actionId)
          : null,
  ...
  const handleFormSubmit = (values: FormValues) => {
    if (mode === 'edit') {
      const editedValues = Object.keys(values).reduce(
        (result: Partial<FormValues>, key) => {
          if (
            values[key as keyof FormValues] !==
            initialData[key as keyof FormValues]
          ) {
            result[key as keyof FormValues] =
              values[key as keyof FormValues];
          }
          return result;
        },
